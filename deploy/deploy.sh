...
#
# Example:
#   ./deploy/deploy.sh vps.example.com deploy /opt/selfhosted-stack core/traefik core/dns business/dolibarr
#
...
\${ENGINE} network create proxy-frontend 2>/dev/null || true

# Determine which stacks to deploy
DEPLOY_STACKS=(${STACKS[*]:-})

if [ \${#DEPLOY_STACKS[@]} -eq 0 ]; then
    # Deploy default stacks
    DEPLOY_STACKS=(core/traefik core/dns core/wireguard business/dolibarr)
fi
